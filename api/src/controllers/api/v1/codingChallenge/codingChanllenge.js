...
const show = async (request, response) => {
    try {
        ...
        return response.status(HTTP_OK).send(success(codingChallenge, 'Coding Challenge fetched successfully', HTTP_OK));
    } catch (exception) {
        logger.error(`Coding Challenge fetching : ${exception.message} `);
        return response.status(HTTP_INTERNAL_SERVER_ERROR).send(error(exception.message));
    }
}
